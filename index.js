...
app.use((req, res, next) => {
 res.set('Access-Control-Allow-Origin', 'http://localhost:4100'); // this is the rocket.chat URL
...
app.post('/sso', function (req, res) {
 ...
 if (!currentUsername) {
 axios.post('http://localhost:4100/api/v1/users.register', {
 username: 'new-user',
 ...
 return axios.post('http://localhost:4100/api/v1/login', {
 username: 'new-user',
 ...
 axios.post('http://localhost:4100/api/v1/login', {
 username: 'new-user',
 password: 'new-users-passw0rd'
 }).then(function (response) {
 if (response.data.status === 'success') {
...
app.post('/login', function (req, res) {
 ...
 loginToken: '${ response.data.data.authToken }'
 }, 'http://localhost:4100'); // rocket.chat's URL
 </script>`);
 }
 }).catch(function() {
 res.sendStatus(401);
